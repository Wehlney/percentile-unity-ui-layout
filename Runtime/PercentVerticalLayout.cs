...
namespace Extensions.Layout
{
    ...
    public sealed class PercentVerticalLayoutAnchors : LayoutGroup
    {
        // =====================================================================
        // Child Rule
        // =====================================================================
        ...
        public sealed class ChildRule
        {
            ...
            [Header("Extra Padding (percent of parent INNER height)")]
            [Range(0f, 200f)] public float extraPaddingTopPercent = 0f;
            [Range(0f, 200f)] public float extraPaddingBottomPercent = 0f;

            [Header("Cross-axis Inset (percent of parent INNER width)")]
            [Range(0f, 200f)] public float insetLeftPercent = 0f;
            [Range(0f, 200f)] public float insetRightPercent = 0f;
        ...
        private void ApplyLayout()
        {
            ...
            float spacingNorm = PercentOf(innerH, spacingPercent);
            float totalSpacingNorm = TotalInterItemSpacing(activeCount, spacingNorm);

            float availableInnerH = RemainingAfter(innerH, totalSpacingNorm);

            float cursorTop = inner.top;
            ...
            for (int i = 0; i < children.Count; i++)
            {
                ...
                float heightNorm = ScaledByPercent(availableInnerH, sizePct, rule.sizeMultiplier);

                float extraTopNorm = ScaledByPercent(innerH, rule.extraPaddingTopPercent, rule.paddingMultiplier);
                float extraBottomNorm = ScaledByPercent(innerH, rule.extraPaddingBottomPercent, rule.paddingMultiplier);

                float insetLeftNorm = ScaledByPercent(innerW, rule.insetLeftPercent, rule.insetMultiplier);
                float insetRightNorm = ScaledByPercent(innerW, rule.insetRightPercent, rule.insetMultiplier);

                cursorTop = Add(cursorTop, -extraTopNorm);

                float childTop = cursorTop;
                float childBottom = childTop - heightNorm;

                SetChildAnchors(child, inner, insetLeftNorm, insetRightNorm, childTop, childBottom);
                ForceZ(child, 0f);

                cursorTop = Add(childBottom, -extraBottomNorm);

                remaining--;
                if (remaining > 0)
                    cursorTop = Add(cursorTop, -spacingNorm);
            }
        }
        ...
        private static void SetChildAnchors(
            ...
            float insetLeftNorm,
            float insetRightNorm,
            float top,
            float bottom)
        {
            float xMin = Clamp01(inner.left + insetLeftNorm);
            float xMax = Clamp01(inner.right - insetRightNorm);

            xMax = Max(xMax, xMin);
            top = Max(top, bottom);

            child.anchorMin = new Vector2(xMin, bottom);
            child.anchorMax = new Vector2(xMax, top);
...
#if UNITY_EDITOR
    [CustomPropertyDrawer(typeof(PercentVerticalLayoutAnchors.ChildRule))]
    internal sealed class PercentVerticalLayoutAnchorsChildRuleDrawer : PropertyDrawer
    {
